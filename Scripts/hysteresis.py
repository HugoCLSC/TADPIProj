...
class Hysteresis():
    ...
    def setup(self):
        try:
            ...
            _file = open(os.getcwd() + "\\Scripts\\hysteresis.cl", "r")
    ...
    def GPUCalc(self, image: np.ndarray, threshold1=0.05, threshold2=0.09):
        try:
            ...
            kernelName = prog.hysteresis_image2D
            ...
            kernelName1.set_arg(0, imageIn)
            kernelName1.set_arg(1, imageOut)
            kernelName1.set_arg(2, np.int32(padding))
            kernelName1.set_arg(3, np.float32(threshold1))
            kernelName1.set_arg(4, np.float32(threshold2))
            kernelName1.set_arg(5, np.int32(width))
            kernelName1.set_arg(6, np.int32(height))
            kernelName1.set_arg(7, np.float32(imageBGRA.max()))
